...
```{r eval=FALSE}
    setwd("/Users/jacobmunozc/Downloads")
```
...
```{r comment=NA}
    US90<-read.table("/Users/jacobmunozc/Downloads/US90.txt", sep="", header=TRUE)
```
...
```{r comment=NA}
    ...
    model2<-lm(gdpgr~consgr+invgr+producgr+unemp+inf, data = US90)
    summary(model2)
```
